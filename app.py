...
def run_main_cmd(args_list, timeout=30):
    ...
    """
    try:
        # Use shell=False and list form to avoid shell injection
        proc = subprocess.run(
            args_list,
            cwd=str(ROOT),
...
def api_analyze():
    ...
    # If your main.py writes machine-readable JSON to stdout, return that.
    if res["ok"]:
        # try parsing stdout as JSON and return that object if possible
        try:
...
if __name__ == "__main__":
    # Use 0.0.0.0 in dev if you want external access
