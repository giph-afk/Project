...
Features:
- Uses zxcvbn (if installed) for advanced pattern-based analysis.
- Falls back to Shannon-entropy-based estimator when zxcvbn is unavailable.
- Combines:
    * Entropy (bits)
    * Estimated crack time
    * Character set size
    * Simple structural checks (length, repetition, common words)
- CLI modes: single password, file, interactive.
- JSON output mode for integration with web UI / other tools.

Exposed API:
    analyze_password(password: str, user_inputs: List[str] | None = None) -> dict
"""
